...
const NavBar = () => {
  ...
  return (
    ...
    >
      ...
      <div
        onClick={() => setNav(!nav)}
        className=" cursor-pointer text-gray-500  p-2 z-10 md:hidden"
      >
        {nav ? <FaTimes size={30} /> : <FaBars size={30} />}
      </div>

      {nav && (
        <ul className=" flex flex-col justify-center items-center w-full h-screen absolute top-0 left-0 text-gray-500 bg-gradient-to-b from-black to-gray-800 md:hidden">
          {links.map(({ id, link }) => (
            <li
              key={id}
